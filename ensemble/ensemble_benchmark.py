...
def benchmark_ensemble(datasets=None, methods=None, combine=None, random_state=None):
    '''
    ...
    for item in datasets:
        ...
        if isinstance(item, tuple):
            ...
            #aucs.mean(axis=0)
            #aucs.std(axis=0)
            rows.append(["{:.2f}+/-{:.2f}".format(mu, sigma) for (mu, sigma) in zip(aucs.mean(axis=0), aucs.std(axis=0))])
