...
def combine_scores_experimental(matrix_of_scores, method='experimental', **kwargs):
    '''
    Input:
    ...
    if method == 'avg':
        ...
        while detectors:
            ...
            # Find detector with lowest ABSOLUTE (this is the experimental part) correlation to the current ensemble
            corrs= [weighted_pearson_correlation(target_vec, matrix_of_scores[:, j], w) for j in detectors_]
            i = detectors_[np.argmin(np.abs(corrs))]
